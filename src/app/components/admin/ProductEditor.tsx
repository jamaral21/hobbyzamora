...
import { buildSkuFromCategory, isAutoGeneratedSku, normalizeStoreCategory } from '../../lib/sku';
...
export function ProductEditor({
  ...
  const buildInitialFormState = (): Partial<Product> => (
    (product
      ? { ...product, category: normalizeStoreCategory(product.category || '') }
      : {
      sku: '',
      name: '',
      category: normalizeStoreCategory(defaultCategory || ''),
      price: 0,
      cost: 0,
      stock: 0,
      initialStock: 0,
  ...
  const [formData, setFormData] = useState<Partial<Product>>(buildInitialFormState());
  ...
  const sectionGroups = buildSectionGroups(sectionData || []);
  const allowedCategories = new Set<string>(
    sectionGroups.flatMap((group) => [group.parentCategory, ...group.children.map((child) => child.name)])
  );
  const hasLegacyCategory = Boolean(formData.category && !allowedCategories.has(String(formData.category)));
  ...
  useEffect(() => {
    ...
    if (isExistingProduct || !category) return;

    let isActive = true;
    setIsGeneratingSku(true);

    productsAPI.getSuggestedSku(category)
      .then(({ sku }) => {
        if (!isActive) return;
        setFormData((current) => {
          const currentSku = String(current.sku || '').trim();
          if (currentSku && !isAutoGeneratedSku(currentSku)) {
            return current;
          }
          return { ...current, sku };
        });
      ...
      .catch(() => {
        ...
        setFormData((current) => {
          ...
          return { ...current, sku: buildSkuFromCategory(category, 1) };
        });
      })
      .finally(() => {
        if (isActive) setIsGeneratingSku(false);
      });

    return () => {
      isActive = false;
    };
  }, [formData.category, isExistingProduct]);
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <Input
                  ...
                  hint={isExistingProduct
                    ? 'Puedes editarlo manualmente si lo necesitas.'
                    : (isGeneratingSku ? 'Generando SKU según la categoría...' : 'Se genera automáticamente según la categoría seleccionada.')}
                />
                <Select
                  label="Estado"
                ...
                >
                  ...
                  {sectionGroups.map((group) => (
                    <optgroup key={group.parentCategory} label={group.parentCategory}>
                      <option value={group.parentCategory}>{group.parentCategory}</option>
              ...
              {hasLegacyCategory && (
                <div className="rounded-lg border border-amber-500/20 bg-amber-500/10 px-3 py-2 text-xs text-amber-700">
                  La categoría actual no pertenece al menú oficial. Debes elegir una de las categorías visibles en el selector.
                </div>
              )}
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-4 gap-4">
                ...
                <Input
                  ...
                  hint="Si cambias este valor, se recalculará el historial asociado a este SKU."
                  required
                />
                <Input
                  label="Stock"
                  ...
                  {formData.presaleEndDate && (
                    <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-lg">
                      <p className="text-xs text-amber-400">
                        La preventa se cerrará automáticamente el {formatChileDate(formData.presaleEndDate, { day: 'numeric', month: 'long', year: 'numeric' })} o cuando se agoten las unidades.
                      </p>
                    </div>
                  )}
                </div>
