...
router.post('/getnet/query', optionalAuth, async (req, res) => {
    try {
        ...
        const queryResponse = await fetch(`${GETNET_ENDPOINT}/api/session/${payment.getnetPaymentId}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
router.post('/getnet/callback', async (req, res) => {
    try {
        ...
        const queryResponse = await fetch(`${GETNET_ENDPOINT}/api/session/${requestId}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ auth: generatePlacetoPayAuth() }),
        });
...
router.post('/manual', authenticate, requireRole('ADMIN', 'STAFF'), async (req, res) => {
    try {
