    ...
    try {
        ...
        if (search) {
            where.OR = [
                ...
                { instagramUserId: { contains: search } },
            ];
        }
...
router.post('/conversations/:id/messages', authenticate, requireRole('ADMIN', 'STAFF'), async (req, res) => {
    try {
...
router.post('/conversations/:id/takeover', authenticate, requireRole('ADMIN', 'STAFF'), async (req, res) => {
    try {
        const id = req.params.id;
...
// Return to bot
router.post('/conversations/:id/return-to-bot', authenticate, requireRole('ADMIN', 'STAFF'), async (req, res) => {
    try {
        const id = req.params.id;
        const conversation = await prisma.instagramConversation.update({
            where: { id },
...
router.patch('/conversations/:id/status', authenticate, requireRole('ADMIN', 'STAFF'), async (req, res) => {
    try {
...
async function generateBotResponse(message, conversationId) {
    ...
    if (lowerMessage.includes('precio') || lowerMessage.includes('price') || lowerMessage.includes('cuanto') || lowerMessage.includes('how much')) {
        ...
        if (products.length > 0) {
            const productList = products.map(p => `• ${p.name}: $${parseFloat(p.price.toString()).toFixed(2)}`).join('\n');
...
router.post('/conversations/:id/create-order', authenticate, requireRole('ADMIN', 'STAFF'), async (req, res) => {
    try {
