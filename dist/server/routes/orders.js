...
router.get('/', authenticate, requireRole('ADMIN', 'STAFF'), async (req, res) => {
    try {
        const { status, source, startDate, endDate, search, page = '1', limit = '50' } = req.query;
        const where = {};
        if (status) {
            where.status = status;
        }
...
router.post('/', optionalAuth, async (req, res) => {
    try {
        const { items, customerName, customerEmail, customerPhone, shipping, shippingAddress, paymentMethod, } = req.body;
